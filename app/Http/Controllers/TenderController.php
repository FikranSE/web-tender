...
class TenderController extends Controller
{
  ...
  public function index()
  {
    ...
    $tenders = Tender::with('paket')->orderBy('created_at', 'DESC')->get();

    return view('tenders.index', compact('tenders'));
  }

  public function downloadDocPem(Tender $tender)
  {
    if (!$tender->dokumen_pemilihan) {
      abort(404, 'Document not found');
    }

    $filePath = $tender->dokumen_pemilihan;
  ...
  public function downloadBerita(Tender $tender)
  {
    if (!$tender->berita_acara) {
      abort(404, 'Document not found');
    }

    $filePath = $tender->berita_acara;
