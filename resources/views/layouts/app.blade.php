...
<body>
    <header class="px-4 py-2 shadow">
        <div class="flex justify-between">
            ...
            <div class="flex items-center">
                <button data-messages class="p-3 mr-2 focus:outline-none hover:bg-gray-200 hover:rounded-md"
                    typle="button">
                ...
                <button data-dropdown
                    ...
                    <span class="ml-4 text-sm hidden md:inline-block">
                        @auth
                            {{ Auth::user()->name }}
                        @else
                            Guest
    ...
    <div class="flex flex-row">
      <div class="flex flex-col w-64 h-screen overflow-y-auto bg-gray-900 border-r rtl:border-r-0 rtl:border-l dark:bg-gray-900 dark:border-gray-700">
          <div class="sidebar text-center bg-gray-900">
              ...
              <div class="relative">
                  <div id="paketDropdown" class="p-2.5 mt-3 flex items-center rounded-md px-4 duration-300 cursor-pointer hover:bg-blue-600 text-white">
                      <i class="bi bi-bookmark-fill"></i>
                      <span class="text-[15px] ml-4 text-gray-200 font-bold">Management</span>
                      <i class="bi bi-chevron-down ml-auto"></i>
                  </div>
                  <div id="paketSubmenu" class="hidden pl-5 mt-2">
                      <a href="{{ route('admin/products') }}" class="p-2.5 flex items-center rounded-md px-4 duration-300 cursor-pointer hover:bg-blue-700 text-white">
                          <span class="text-[14px] ml-4 text-gray-200">Paket</span>
                      </a>
                      <a href="{{ route('admin/tender') }}" class="p-2.5 flex items-center rounded-md px-4 duration-300 cursor-pointer hover:bg-blue-700 text-white">
                          <span class="text-[14px] ml-4 text-gray-200">Tender</span>
                      </a>
                      <a href="{{ route('penawaran.index') }}" class="p-2.5 flex items-center rounded-md px-4 duration-300 cursor-pointer hover:bg-blue-700 text-white">
                          <span class="text-[14px] ml-4 text-gray-200">Penawaran</span>
                      </a>
                  </div>
              </div>
  ...
  document.addEventListener('DOMContentLoaded', function() {
      ...
      const paketSubmenu = document.getElementById('paketSubmenu');
      
      paketDropdown.addEventListener('click', function() {
          paketSubmenu.classList.toggle('hidden');
          paketDropdown.querySelector('.bi-chevron-down').classList.toggle('rotate-180');
      });
  });
  </script>
